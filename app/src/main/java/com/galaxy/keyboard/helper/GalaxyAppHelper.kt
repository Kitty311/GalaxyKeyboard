...
class GalaxyAppHelper {
    companion object {
        ...
        fun GetCurrentAppTextSize(context: Context): Int {
            ...
            return curFontSize / context.resources.displayMetrics.scaledDensity.toInt()
        }
        ...
        fun CheckIfLicenseAvailable(): Boolean {
            ...
            var formattedDate: String = dateFormat.format(curDate.time)
            Log.e("XianZai", formattedDate)
            val expireDate = Calendar.getInstance()
            expireDate.set(2024, 1, 1)
            formattedDate = dateFormat.format(expireDate.time)
