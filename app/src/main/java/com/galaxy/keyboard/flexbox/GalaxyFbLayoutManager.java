package com.galaxy.keyboard.flexbox;
 ...
 */
public class GalaxyFbLayoutManager extends RecyclerView.LayoutManager implements GalaxyFbContainer,
        RecyclerView.SmoothScroller.ScrollVectorProvider {

    private static final String TAG = "GalaxyFbLayoutManager";
     ...
     * FlexDirection#ROW}.
     *
     * @see GalaxyFbContainer#getFlexDirection()
     */
     ...
     *
     * @see GalaxyFbContainer#getFlexWrap()
     */
     ...
     *
     * @see GalaxyFbContainer#getJustifyContent()
     */
     ...
     *
     * @see GalaxyFbContainer#getAlignItems()
     */
     ...
     */
    private final boolean mFromBottomToTop = true;

    private List<GalaxyFbLine> mFlexLines = new ArrayList<>();

    private final GalaxyFbHelper mFlexboxHelper = new GalaxyFbHelper(this);
    ...
    private GalaxyFbLayoutManager.LayoutState mLayoutState;

    private final GalaxyFbLayoutManager.AnchorInfo mAnchorInfo = new GalaxyFbLayoutManager.AnchorInfo();
    ...
    private GalaxyFbLayoutManager.SavedState mPendingSavedState;
     ...
     */
    private final GalaxyFbHelper.FlexLinesResult mFlexLinesResult = new GalaxyFbHelper.FlexLinesResult();

    /**
     * Creates a default GalaxyFbLayoutManager.
     */
    public GalaxyFbLayoutManager(Context context) {
        this(context, FlexDirection.ROW, FlexWrap.WRAP);
    }

    /**
     * Creates a GalaxyFbLayoutManager with the flexDirection specified.
     *
     * @param flexDirection the flex direction attribute
     */
    public GalaxyFbLayoutManager(Context context, @FlexDirection int flexDirection) {
        this(context, flexDirection, FlexWrap.WRAP);
    }

    /**
     * Creates a GalaxyFbLayoutManager with the flexDirection and flexWrap attributes specified.
     *
     * @param flexDirection the flex direction attribute
     ...
     */
    public GalaxyFbLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                                int defStyleRes) {
    ...
    // From here, methods from GalaxyFbContainer
    @FlexDirection
    @Override
...
//        }
    ...
    public void setAlignContent(@AlignContent int alignContent) {
        throw new UnsupportedOperationException("Setting the alignContent in the "
                + "GalaxyFbLayoutManager is not supported. Use FlexboxLayout "
    ...
    @Override
    @NonNull
    public List<GalaxyFbLine> getFlexLines() {
        List<GalaxyFbLine> result = new ArrayList<>(mFlexLines.size());
        for (int i = 0, size = mFlexLines.size(); i < size; i++) {
            GalaxyFbLine flexLine = mFlexLines.get(i);
    ...
    @Override
    public void onNewFlexItemAdded(View view, int index, int indexInFlexLine, GalaxyFbLine flexLine) {
     ...
     * i.e. {@link GalaxyFbLayoutManager#getChildCount()} returns the child count, but it doesn't
     ...
     *
     * This method is used to avoid the implementation of the similar method.
     * i.e. {@link GalaxyFbLayoutManager#getChildAt(int)} returns a view for the given index,
     ...
     * The order attribute ({@link GalaxyFbItem#getOrder()}) is not supported by this class since
     ...
     * {@link GalaxyFbContainer} interface.
     *
    ...
    @Override
    public void onNewFlexLineAdded(GalaxyFbLine flexLine) {
        // No op
    }
    ...
    @Override
    public void setFlexLines(List<GalaxyFbLine> flexLines) {
        mFlexLines = flexLines;
    }

    @Override
    public List<GalaxyFbLine> getFlexLinesInternal() {
        return mFlexLines;
    }
    ...
    // The end of methods from GalaxyFbContainer
    ...
    public RecyclerView.LayoutParams generateDefaultLayoutParams() {
        return new GalaxyFbLayoutManager.LayoutParams(GalaxyFbLayoutManager.LayoutParams.WRAP_CONTENT, GalaxyFbLayoutManager.LayoutParams.WRAP_CONTENT);
    }

    @Override
    public RecyclerView.LayoutParams generateLayoutParams(Context c, AttributeSet attrs) {
        return new GalaxyFbLayoutManager.LayoutParams(c, attrs);
    }

    @Override
    public boolean checkLayoutParams(RecyclerView.LayoutParams lp) {
        return lp instanceof GalaxyFbLayoutManager.LayoutParams;
    }
    ...
    public Parcelable onSaveInstanceState() {
        if (mPendingSavedState != null) {
            return new GalaxyFbLayoutManager.SavedState(mPendingSavedState);
        }
        GalaxyFbLayoutManager.SavedState savedState = new GalaxyFbLayoutManager.SavedState();
        if (getChildCount() > 0) {
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof GalaxyFbLayoutManager.SavedState) {
            mPendingSavedState = (GalaxyFbLayoutManager.SavedState) state;
            requestLayout();
            if (DEBUG) {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        // the method from GalaxyFbContainer (such as getFlexItemCount()) returns the number of
    ...
    private void updateFlexLines(int childCount) {
        ...
        if (mDirtyPosition == NO_POSITION &&
            ...
            mFlexboxHelper.determineMainSize(widthMeasureSpec, heightMeasureSpec,
                    fromIndex);
            // Unlike the FlexboxLayout not calling GalaxyFbHelper#determineCrossSize because
...
//                mFromBottomToTop = false;
    ...
    private void updateAnchorInfoForLayout(RecyclerView.State state, GalaxyFbLayoutManager.AnchorInfo anchorInfo) {
    ...
    private boolean updateAnchorFromPendingState(RecyclerView.State state, GalaxyFbLayoutManager.AnchorInfo anchorInfo,
                                                 GalaxyFbLayoutManager.SavedState savedState) {
        assert mFlexboxHelper.mIndexToFlexLine != null;
     ...
     */
    private boolean updateAnchorFromChildren(RecyclerView.State state, GalaxyFbLayoutManager.AnchorInfo anchorInfo) {
        if (getChildCount() == 0) {
            return false;
        }
    ...
    private View findFirstReferenceChild(int itemCount) {
        ...
        GalaxyFbLine firstFoundLine = mFlexLines.get(firstFoundLinePosition);
    ...
    private View findLastReferenceChild(int itemCount) {
        ...
        GalaxyFbLine lastFoundLine = mFlexLines.get(lastFoundLinePosition);
     ...
     * how many pixels should be filled (defined by {@link GalaxyFbLayoutManager.LayoutState#mAvailable}.
     * The large part refers to the LinearLayoutManager#fill method except for the fill direction.
     * Because GalaxyFbLayoutManager needs to care two scrolling directions:
     * <li>
    ...
    private int fill(RecyclerView.Recycler recycler, RecyclerView.State state,
                     GalaxyFbLayoutManager.LayoutState layoutState) {
        if (layoutState.mScrollingOffset != GalaxyFbLayoutManager.LayoutState.SCROLLING_OFFSET_NaN) {
        ...
        while ((remainingSpace > 0 || mLayoutState.mInfinite) &&
            ...
            GalaxyFbLine flexLine = mFlexLines.get(layoutState.mFlexLinePosition);
    ...
    private void recycleByLayoutState(RecyclerView.Recycler recycler, GalaxyFbLayoutManager.LayoutState layoutState) {
        if (!layoutState.mShouldRecycle) {
            return;
        }
        if (layoutState.mLayoutDirection == GalaxyFbLayoutManager.LayoutState.LAYOUT_START) {
    ...
    private void recycleFlexLinesFromStart(RecyclerView.Recycler recycler,
        ...
        GalaxyFbLine flexLine = mFlexLines.get(currentLineIndex);
        int recycleTo = -1;
    ...
    private void recycleFlexLinesFromEnd(RecyclerView.Recycler recycler, GalaxyFbLayoutManager.LayoutState layoutState) {
        if (layoutState.mScrollingOffset < 0) {
            return;
        }
        assert mFlexboxHelper.mIndexToFlexLine != null;
    ...
    private int layoutFlexLine(GalaxyFbLine flexLine, GalaxyFbLayoutManager.LayoutState layoutState) {
        if (isMainAxisDirectionHorizontal()) {
    ...
    private int layoutFlexLineMainAxisHorizontal(GalaxyFbLine flexLine, GalaxyFbLayoutManager.LayoutState layoutState) {
        ...
        for (int i = startPosition, itemCount = flexLine.getItemCount();
            ...
            if (layoutState.mLayoutDirection == GalaxyFbLayoutManager.LayoutState.LAYOUT_END) {
                calculateItemDecorationsForChild(view, TEMP_RECT);
                addView(view);
            } else {
                calculateItemDecorationsForChild(view, TEMP_RECT);
            ...
            // each visible child should be measured at least once in the GalaxyFbHelper
            ...
            GalaxyFbLayoutManager.LayoutParams lp = (GalaxyFbLayoutManager.LayoutParams) view.getLayoutParams();
    ...
    private int layoutFlexLineMainAxisVertical(GalaxyFbLine flexLine, GalaxyFbLayoutManager.LayoutState layoutState) {
        assert mFlexboxHelper.mMeasureSpecCache != null;
     ...
     */
    private void updateLayoutStateToFillEnd(GalaxyFbLayoutManager.AnchorInfo anchorInfo, boolean fromNextLine,
        ...
        mLayoutState.mItemDirection = GalaxyFbLayoutManager.LayoutState.ITEM_DIRECTION_TAIL;
        mLayoutState.mLayoutDirection = GalaxyFbLayoutManager.LayoutState.LAYOUT_END;
        mLayoutState.mOffset = anchorInfo.mCoordinate;
        mLayoutState.mScrollingOffset = GalaxyFbLayoutManager.LayoutState.SCROLLING_OFFSET_NaN;
        ...
        if (fromNextLine
            ...
            GalaxyFbLine currentLine = mFlexLines.get(anchorInfo.mFlexLinePosition);
     ...
     *                         needs to be considered.
     */
    private void updateLayoutStateToFillStart(GalaxyFbLayoutManager.AnchorInfo anchorInfo, boolean fromPreviousLine,
                                              boolean considerInfinite) {
        if (considerInfinite) {
            resolveInfiniteAmount();
        } else {
        ...
        mLayoutState.mPosition = anchorInfo.mPosition;
        mLayoutState.mItemDirection = GalaxyFbLayoutManager.LayoutState.ITEM_DIRECTION_TAIL;
        mLayoutState.mLayoutDirection = GalaxyFbLayoutManager.LayoutState.LAYOUT_START;
        mLayoutState.mOffset = anchorInfo.mCoordinate;
        mLayoutState.mScrollingOffset = GalaxyFbLayoutManager.LayoutState.SCROLLING_OFFSET_NaN;
        mLayoutState.mFlexLinePosition = anchorInfo.mFlexLinePosition;
    ...
    private void ensureLayoutState() {
        if (mLayoutState == null) {
            mLayoutState = new GalaxyFbLayoutManager.LayoutState();
        }
    }
    ...
    private int handleScrollingMainOrientation(int delta, RecyclerView.Recycler recycler,
        ...
        boolean columnAndRtl = !isMainAxisDirectionHorizontal() && mIsRtl;
        if (columnAndRtl) {
            layoutDirection = delta < 0 ? GalaxyFbLayoutManager.LayoutState.LAYOUT_END : GalaxyFbLayoutManager.LayoutState.LAYOUT_START;
        } else {
            layoutDirection = delta > 0 ? GalaxyFbLayoutManager.LayoutState.LAYOUT_END : GalaxyFbLayoutManager.LayoutState.LAYOUT_START;
        }
     ...
     *
     * @param layoutDirection the layout direction value. Either of {@link GalaxyFbLayoutManager.LayoutState#LAYOUT_END}
     *                        or {@link GalaxyFbLayoutManager.LayoutState#LAYOUT_START}
    ...
    private void updateLayoutState(int layoutDirection, int absDelta) {
        ...
        if (layoutDirection == GalaxyFbLayoutManager.LayoutState.LAYOUT_END) {
            ...
            GalaxyFbLine lastVisibleLine = mFlexLines.get(lastVisibleLinePosition);
            ...
            GalaxyFbLine firstVisibleLine = mFlexLines.get(firstVisibleLinePosition);
            ...
            if (flexLinePosition > 0) {
                GalaxyFbLine previousLine = mFlexLines.get(flexLinePosition - 1);
     ...
     * @return the reference view
     */
    private View findFirstReferenceViewInLine(View firstView, GalaxyFbLine firstVisibleLine) {
        boolean mainAxisHorizontal = isMainAxisDirectionHorizontal();
     ...
     * @return the reference view
     */
    private View findLastReferenceViewInLine(View lastView, GalaxyFbLine lastVisibleLine) {
        boolean mainAxisHorizontal = isMainAxisDirectionHorizontal();
     ...
     * @return the index of the {@link GalaxyFbLine}, which includes the view whose index is passed as
    ...
    /**
     * LayoutParams used by the {@link GalaxyFbLayoutManager}, which stores per-child information
     ...
     */
    public static class LayoutParams extends RecyclerView.LayoutParams implements GalaxyFbItem {

        /**
         * @see GalaxyFbItem#getFlexGrow()
         */
        private float mFlexGrow = GalaxyFbItem.FLEX_GROW_DEFAULT;

        /**
         * @see GalaxyFbItem#getFlexShrink()
         */
        private float mFlexShrink = GalaxyFbItem.FLEX_SHRINK_DEFAULT;

        /**
         * @see GalaxyFbItem#getAlignSelf()
         */
        private int mAlignSelf = AlignSelf.AUTO;

        /**
         * @see GalaxyFbItem#getFlexBasisPercent()
         */
        private float mFlexBasisPercent = GalaxyFbItem.FLEX_BASIS_PERCENT_DEFAULT;

        /**
         * @see GalaxyFbItem#getMinWidth()
         */
        private int mMinWidth;

        /**
         * @see GalaxyFbItem#getMinHeight()
         */
        private int mMinHeight;

        /**
         * @see GalaxyFbItem#getMaxWidth()
         */
        private int mMaxWidth = MAX_SIZE;

        /**
         * @see GalaxyFbItem#getMaxHeight()
         */
        private int mMaxHeight = MAX_SIZE;

        /**
         * @see GalaxyFbItem#isWrapBefore()
         */
        ...
        public LayoutParams(GalaxyFbLayoutManager.LayoutParams source) {
            super(source);
        ...
        @Override
        public int getOrder() {
            return GalaxyFbItem.ORDER_DEFAULT;
        }
        ...
        public static final Parcelable.Creator<GalaxyFbLayoutManager.LayoutParams> CREATOR
                = new Parcelable.Creator<GalaxyFbLayoutManager.LayoutParams>() {
            @Override
            public GalaxyFbLayoutManager.LayoutParams createFromParcel(Parcel source) {
                return new GalaxyFbLayoutManager.LayoutParams(source);
            }

            @Override
            public GalaxyFbLayoutManager.LayoutParams[] newArray(int size) {
                return new GalaxyFbLayoutManager.LayoutParams[size];
            }
        };
    }
    ...
    /**
     * Helper class that keeps temporary state while the GalaxyFbLayoutManager is filling out the
     * empty space.
     */
    private static class LayoutState {
        ...
        private int mItemDirection = GalaxyFbLayoutManager.LayoutState.ITEM_DIRECTION_TAIL;

        private int mLayoutDirection = GalaxyFbLayoutManager.LayoutState.LAYOUT_END;
         ...
         */
        private boolean hasMore(RecyclerView.State state, List<GalaxyFbLine> flexLines) {
    ...
    private static class SavedState implements Parcelable {
        ...
        private SavedState(GalaxyFbLayoutManager.SavedState savedState) {
        ...
        public static final Creator<GalaxyFbLayoutManager.SavedState> CREATOR = new Creator<GalaxyFbLayoutManager.SavedState>() {
            @Override
            public GalaxyFbLayoutManager.SavedState createFromParcel(Parcel source) {
                return new GalaxyFbLayoutManager.SavedState(source);
            }

            @Override
            public GalaxyFbLayoutManager.SavedState[] newArray(int size) {
                return new GalaxyFbLayoutManager.SavedState[size];
            }
        };
