...
import static com.galaxy.keyboard.flexbox.GalaxyFbContainer.NOT_SET;
import static com.galaxy.keyboard.flexbox.GalaxyFbItem.FLEX_BASIS_PERCENT_DEFAULT;
import static com.galaxy.keyboard.flexbox.GalaxyFbItem.FLEX_GROW_DEFAULT;
import static com.galaxy.keyboard.flexbox.GalaxyFbItem.FLEX_SHRINK_NOT_SET;
...
class GalaxyFbHelper {
     ...
     * <p>
     * GalaxyFbLine(0): itemCount 3
     * GalaxyFbLine(1): itemCount 2
     * </p>
    ...
    GalaxyFbHelper(GalaxyFbContainer flexContainer) {
        mFlexContainer = flexContainer;
    }

    /**
     * Create an array, which indicates the reordered indices that
     * {@link GalaxyFbItem#getOrder()} attributes are taken into account.
    ...
    int[] createReorderedIndices(View viewBeforeAdded, int indexForViewBeforeAdded,
        ...
        List<GalaxyFbHelper.Order> orders = createOrders(childCount);
        GalaxyFbHelper.Order orderForViewToBeAdded = new GalaxyFbHelper.Order();
        if (viewBeforeAdded != null
                && paramsForViewBeforeAdded instanceof GalaxyFbItem) {
            orderForViewToBeAdded.order = ((GalaxyFbItem)
                    paramsForViewBeforeAdded).getOrder();
        } else {
            orderForViewToBeAdded.order = GalaxyFbItem.ORDER_DEFAULT;
        }
    ...
    @NonNull
    private List<GalaxyFbHelper.Order> createOrders(int childCount) {
        List<GalaxyFbHelper.Order> orders = new ArrayList<>(childCount);
        for (int i = 0; i < childCount; i++) {
            View child = mFlexContainer.getFlexItemAt(i);
            GalaxyFbItem flexItem = (GalaxyFbItem) child.getLayoutParams();
            GalaxyFbHelper.Order order = new GalaxyFbHelper.Order();
    ...
    /**
     * Returns if any of the children's {@link GalaxyFbItem#getOrder()} attributes are
    ...
    boolean isOrderChangedFromLastMeasurement(SparseIntArray orderCache) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            GalaxyFbItem flexItem = (GalaxyFbItem) view.getLayoutParams();
    ...
    private int[] sortOrdersIntoReorderedIndices(int childCount, List<GalaxyFbHelper.Order> orders,
                                                 SparseIntArray orderCache) {
        Collections.sort(orders);
        orderCache.clear();
        int[] reorderedIndices = new int[childCount];
        int i = 0;
        for (GalaxyFbHelper.Order order : orders) {
     ...
     *
     * @see #calculateFlexLines(GalaxyFbHelper.FlexLinesResult, int, int, int, int, int, List)
     */
    void calculateHorizontalFlexLines(GalaxyFbHelper.FlexLinesResult result, int widthMeasureSpec,
    ...
    void calculateHorizontalFlexLines(GalaxyFbHelper.FlexLinesResult result, int widthMeasureSpec,
                                      ...
                                      @Nullable List<GalaxyFbLine> existingLines) {
        calculateFlexLines(result, widthMeasureSpec, heightMeasureSpec, needsCalcAmount,
                fromIndex, NO_POSITION, existingLines);
    }

    void calculateHorizontalFlexLinesToIndex(GalaxyFbHelper.FlexLinesResult result, int widthMeasureSpec,
                                             int heightMeasureSpec, int needsCalcAmount, int toIndex, List<GalaxyFbLine> existingLines) {
        calculateFlexLines(result, widthMeasureSpec, heightMeasureSpec, needsCalcAmount,
                0, toIndex, existingLines);
    }

    void calculateVerticalFlexLines(GalaxyFbHelper.FlexLinesResult result, int widthMeasureSpec, int heightMeasureSpec) {
        calculateFlexLines(result, heightMeasureSpec, widthMeasureSpec, Integer.MAX_VALUE,
                0, NO_POSITION, null);
    }

    void calculateVerticalFlexLines(GalaxyFbHelper.FlexLinesResult result, int widthMeasureSpec,
                                    int heightMeasureSpec, int needsCalcAmount, int fromIndex,
                                    @Nullable List<GalaxyFbLine> existingLines) {
    ...
    void calculateVerticalFlexLinesToIndex(GalaxyFbHelper.FlexLinesResult result, int widthMeasureSpec,
                                           int heightMeasureSpec, int needsCalcAmount, int toIndex, List<GalaxyFbLine> existingLines) {
        calculateFlexLines(result, heightMeasureSpec, widthMeasureSpec, needsCalcAmount,
                0, toIndex, existingLines);
    }
    ...
    void calculateFlexLines(GalaxyFbHelper.FlexLinesResult result, int mainMeasureSpec,
        ...
        GalaxyFbLine flexLine = new GalaxyFbLine();
        ...
        for (int i = fromIndex; i < childCount; i++) {
            ...
            // {@link GalaxyFbItem#getMinWidth()}, {@link GalaxyFbItem#getMinHeight()},
            // {@link GalaxyFbItem#getMaxWidth()} and {@link GalaxyFbItem#getMaxHeight()} attributes.
    ...
    private void evaluateMinimumSizeForCompoundButton(CompoundButton compoundButton) {
        GalaxyFbItem flexItem = (GalaxyFbItem) compoundButton.getLayoutParams();
     ...
     */
    private int getFlexItemSizeMain(GalaxyFbItem flexItem, boolean isMainHorizontal) {
        if (isMainHorizontal) {
     ...
     */
    private int getFlexItemSizeCross(GalaxyFbItem flexItem, boolean isMainHorizontal) {
        if (isMainHorizontal) {
            return flexItem.getHeight();
        }
     ...
     * {@link GalaxyFbItem#getMarginLeft} (ViewGroup.MarginLayoutParams#getMarginStart isn't available
     ...
     */
    private int getFlexItemMarginStartMain(GalaxyFbItem flexItem, boolean isMainHorizontal) {
        if (isMainHorizontal) {
     ...
     * For the backward compatibility for API level < 17, the horizontal margin is returned using
     * {@link GalaxyFbItem#getMarginRight} (ViewGroup.MarginLayoutParams#getMarginEnd isn't available
     ...
     */
    private int getFlexItemMarginEndMain(GalaxyFbItem flexItem, boolean isMainHorizontal) {
        if (isMainHorizontal) {
     ...
     */
    private int getFlexItemMarginStartCross(GalaxyFbItem flexItem, boolean isMainHorizontal) {
        if (isMainHorizontal) {
            return flexItem.getMarginTop();
        }
     ...
     */
    private int getFlexItemMarginEndCross(GalaxyFbItem flexItem, boolean isMainHorizontal) {
        if (isMainHorizontal) {
            return flexItem.getMarginBottom();
        }
     ...
     * @see GalaxyFbContainer#setFlexWrap(int)
     */
    private boolean isWrapRequired(View view, int mode, int maxSize, int currentLength,
                                   int childLength, GalaxyFbItem flexItem, int index, int indexInFlexLine, int flexLinesSize) {
    ...
    private boolean isLastFlexItem(int childIndex, int childCount,
                                   GalaxyFbLine flexLine) {
        return childIndex == childCount - 1 && flexLine.getItemCountNotGone() != 0;
    }

    private void addFlexLine(List<GalaxyFbLine> flexLines, GalaxyFbLine flexLine, int viewIndex,
     ...
     * by the {@link GalaxyFbItem#getMinWidth()}, {@link GalaxyFbItem#getMinHeight()},
     * {@link GalaxyFbItem#getMaxWidth()} and {@link GalaxyFbItem#getMaxHeight()} attributes.
     *
     ...
     * @see GalaxyFbContainer#setFlexDirection(int)
    ...
    void determineMainSize(int widthMeasureSpec, int heightMeasureSpec, int fromIndex) {
        ...
        List<GalaxyFbLine> flexLines = mFlexContainer.getFlexLinesInternal();
        for (int i = flexLineIndex, size = flexLines.size(); i < size; i++) {
            GalaxyFbLine flexLine = flexLines.get(i);
     ...
     */
    private void expandFlexItems(int widthMeasureSpec, int heightMeasureSpec, GalaxyFbLine flexLine,
     ...
     */
    private void shrinkFlexItems(int widthMeasureSpec, int heightMeasureSpec, GalaxyFbLine flexLine,
                                 int maxMainSize, int paddingAlongMainAxis, boolean calledRecursively) {
    ...
    private int getChildWidthMeasureSpecInternal(int widthMeasureSpec, GalaxyFbItem flexItem,
                                                 int padding) {
    ...
    private int getChildHeightMeasureSpecInternal(int heightMeasureSpec, GalaxyFbItem flexItem,
                                                  int padding) {
     ...
     * @see GalaxyFbContainer#getFlexDirection()
     * @see GalaxyFbContainer#setFlexDirection(int)
     * @see GalaxyFbContainer#getAlignContent()
     * @see GalaxyFbContainer#setAlignContent(int)
     */
            ...
            if (flexLines.size() == 1) {
                ...
                switch (mFlexContainer.getAlignContent()) {
                    ...
                    case AlignContent.SPACE_AROUND: {
                        ...
                        List<GalaxyFbLine> newFlexLines = new ArrayList<>();
                        GalaxyFbLine dummySpaceFlexLine = new GalaxyFbLine();
                        dummySpaceFlexLine.mCrossSize = spaceTopAndBottom;
                        for (GalaxyFbLine flexLine : flexLines) {
    ...
    private List<GalaxyFbLine> constructFlexLinesForAlignContentCenter(List<GalaxyFbLine> flexLines,
    ...
    /**
     * Expand the view if the {@link GalaxyFbContainer#getAlignItems()} attribute is set to {@link
     * AlignItems#STRETCH} or {@link GalaxyFbItem#getAlignSelf()} is set as
     ...
     * @see GalaxyFbContainer#setAlignItems(int)
    ...
    void stretchViews(int fromIndex) {
        ...
        if (mFlexContainer.getAlignItems() == AlignItems.STRETCH) {
            ...
            for (GalaxyFbLine flexLine : mFlexContainer.getFlexLinesInternal()) {
     ...
     * ({@link GalaxyFbContainer#getFlexDirection()} is either {@link FlexDirection#ROW} or
     * {@link FlexDirection#ROW_REVERSE}).
     *
     * @param view     the View to be placed
     * @param flexLine the {@link GalaxyFbLine} where the View belongs to
     ...
     * @see GalaxyFbItem#getAlignSelf()
     */
    void layoutSingleChildHorizontal(View view, GalaxyFbLine flexLine, int left, int top, int right,
                                     int bottom) {
        GalaxyFbItem flexItem = (GalaxyFbItem) view.getLayoutParams();
     ...
     * ({@link GalaxyFbContainer#getFlexDirection()} is either {@link FlexDirection#COLUMN} or
     ...
     * @see GalaxyFbItem#getAlignSelf()
     */
    void layoutSingleChildVertical(View view, GalaxyFbLine flexLine, boolean isRtl,
     ...
     */
    void clearFlexLines(List<GalaxyFbLine> flexLines, int fromFlexItem) {
     ...
     */
    private static class Order implements Comparable<GalaxyFbHelper.Order> {
        ...
        @Override
        public int compareTo(@NonNull GalaxyFbHelper.Order another) {
